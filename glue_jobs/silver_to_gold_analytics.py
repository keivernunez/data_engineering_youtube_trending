...
# CORRECCIÓN 2: Eliminado el stats_df.count() para evitar doble escaneo del S3
logger.info("Statistics dataframe loaded in DAG.")
...
try:
    ...
    if "id" in ref_df.columns and "snippet.title" in ref_df.columns:
        ...
        logger.warn(f"Could not find expected category title columns. Columns found: {ref_df.columns}")

    if category_lookup is not None:
        # CORRECCIÓN 2: Eliminado category_lookup.count()
        logger.info("Category lookup dataframe built.")
...
sink1 = glueContext.getSink(
    ...
    # CORRECCIÓN 3: Particionado temporal incluido
...
logger.info(f"Write operation dispatched for trending_analytics → {trending_path}")
...
channel = stats_df.groupBy("channel_title", "region").agg(
    ...
    # CORRECCIÓN 1: Flatten del Array a String separado por comas
    F.concat_ws(", ", F.collect_set("category_name")).alias("categories"),
)
...
sink2 = glueContext.getSink(
    ...
    enableUpdateCatalog=True,
    updateBehavior="UPDATE_IN_DATABASE",
    partitionKeys=["region"], # Aquí region es suficiente porque no hay granularidad diaria
)
...
logger.info(f"Write operation dispatched for channel_analytics → {channel_path}")
...
logger.info(f"Write operation dispatched for category_analytics → {category_path}")
